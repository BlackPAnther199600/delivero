...
const jitter = (v, meters) => {
  // ~111_111m per degree latitude, longitude depends on latitude
...
const ensureOrderTrackingColumns = async () => {
  // These columns are referenced by controllers; make sure they exist.
...
(async () => {
  try {
    console.log('Seeding demo data near:', CENTER);
    ...
    console.log('Done. You can now open manager MAP / tracking to see nearby data.');
    process.exit(0);
  } catch (e) {
